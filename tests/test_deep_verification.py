...
except: check('logic', '수수료 정의', False, 'Import 실패')

# 시그널 - AlphaX7Core
try:
    from core.strategy_core import AlphaX7Core
    check('logic', 'AlphaX7Core', hasattr(AlphaX7Core, 'check_signal'), 'check_signal 메서드')
except: check('logic', 'AlphaX7Core', False, 'Import 실패')
...
except: check('logic', 'MTF 필터', False, 'Import 실패')

# 진입 조건
try:
    from core.order_executor import OrderExecutor
    check('logic', '진입 조건', hasattr(OrderExecutor, 'execute_entry'), 'execute_entry')
except: check('logic', '진입 조건', False, 'Import 실패')
...
except: check('logic', '트레일링 SL', False, 'Import 실패')
...
except: check('logic', '방향 상수', False, 'Import 실패')
...
except: check('data_flow', '경로 관리', False, 'Import 실패')
...
except: check('error_handling', 'API 재시도', False, 'Import 실패')
...
except: check('error_handling', '상태 복구', False, 'Import 실패')
...
except: check('error_handling', '입력 검증', False, 'Import 실패')
...
except: check('error_handling', '잘못된 심볼', False, 'Import 실패')
...
except: check('error_handling', '경로 검증', False, 'Import 실패')
...
except: check('error_handling', '0 나눗셈', False, 'Import 실패')
...
except: check('error_handling', '캐시 TTL', False, 'Import 실패')
...
except: check('integration', '신규 사용자', False, 'Import 실패')
...
except: check('integration', '일일 운영', False, 'Import 실패')
...
except: check('integration', '장애 복구', False, 'Import 실패')
