...
if os.environ.get('SUPPRESS_INIT_LOGS') != '1':
...
try:
    ...
    logging.error("CRITICAL: constants.py not found in unified_bot.py")
    # No local fallback for DEFAULT_PARAMS - enforce project standard
...
class UnifiedBot:
    ...
    # [SYNC] constants.py와 동일한 값으로 동기화 - 2026.01.01
    DEFAULT_PATTERN_TOLERANCE = 0.05  # [FIX] constants.py와 동기화
    DEFAULT_ENTRY_VALIDITY_HOURS = 48.0  # [SYNC] constants.py와 동기화
    DEFAULT_ATR_MULT = 1.5  # [SYNC] constants.py와 동기화
    DEFAULT_TRAIL_START_R = 0.8  # [SYNC] constants.py와 동기화
    DEFAULT_TRAIL_DIST_R = 0.5  # [SYNC] constants.py와 동기화
    DEFAULT_RSI_PERIOD = 14  # 동일
    DEFAULT_PULLBACK_RSI_LONG = 45  # [FIX] constants.py와 동기화 (40→45)
    DEFAULT_PULLBACK_RSI_SHORT = 55  # [FIX] constants.py와 동기화 (60→55)
    DEFAULT_MAX_ADDS = 1
    ...
    def adjust_capital(self, amount: float):
        ...
        with self._data_lock:
            ...
            # 2. 거래소 자본금(실제 사용 가능액) 조정
            if self.exchange:
                self.exchange.capital += amount
            
            logging.info(f"💰 [CAPITAL ADJ] {amount:+.2f}$ 반영됨. New Capital: ${self.exchange.capital:.2f}")
    ...
    def reset_session(self):
        ...
        """
        with self._data_lock:
            # 1. 거래 내역 초기화 (파일 백업 및 삭제)
            if hasattr(self, 'trade_storage'):
                from storage.trade_storage import get_trade_storage
                self.trade_storage.reset_history()
            
            # 2. 자본금 원복 (누적 PnL을 0으로 간주)
            if self.exchange:
                self.exchange.capital = self.initial_capital
            
            logging.info(f"🧹 [SESSION RESET] PnL 리셋 완료. Capital 원복: ${self.initial_capital:.2f}")
    ...
    def _run_backtest_to_now(self) -> bool:
        ...
        try:
            ...
            # [v1.5.9] 기존 복구된(load_state) 실시간 포지션이 있다면 백테스트 결과보다 우선함
            if self.position is not None:
                logging.info(f"[INIT] Preserving restored position {self.position.side} over backtest result")
                state['position'] = self.position.side
                if not state.get('positions'):
                     state['positions'] = [{'entry': self.position.entry_price, 'size': self.position.size}]
    ...
    def _process_new_candle(self, candle: dict):
        ...
        with self._data_lock:
            try:
                ...
                # 3. 1시간 봉 마감 시 df_pattern_full에 추가 (09:45 봉 마감 시 09:00 봉 생성)
                new_1h_candle = None
                if ts.minute == 45 and self.df_entry_full is not None and len(self.df_entry_full) >= 4:
    ...
    def _execute_live_close(self, action: dict):
        ...
        try:
            ...
            if getattr(self, 'trade_storage', None) and self.position:
                try:
                    ...
                    if self.position.entry_price > 0:
                        leverage = (self.exchange.leverage if self.exchange else 1)
                        # [v1.5.10] Use position.side for correct ROE/PnL sign
                        if self.position.side == 'Long':
                            pnl_pct = (price - self.position.entry_price) / self.position.entry_price * leverage * 100
                        else:
                            pnl_pct = (self.position.entry_price - price) / self.position.entry_price * leverage * 100
                        
                        # [v1.5.10] Size-based PnL calculation (Quantity * PriceDiff)
                        size = self.position.size
                        pnl_usd_raw = size * (price - self.position.entry_price) if self.position.side == 'Long' else size * (self.position.entry_price - price)
                        
                        # [v1.5.10] Deduct Fee (Round-trip: entry + exit)
                        fee_rate = getattr(self, 'slippage', 0.0006) # Default 0.06%
                        total_fee_usd = size * self.position.entry_price * fee_rate + size * price * fee_rate
                        pnl_usd = pnl_usd_raw - total_fee_usd
            ...
            if self.position:
                ...
                # [v1.5.10] Standardized PnL calculation matching storage
                if self.position.side == 'Long':
                    pnl_pct = (price - entry) / entry * leverage * 100
                else:
                    pnl_pct = (entry - price) / entry * leverage * 100
                    
                # [v1.5.10] Size-based Profit Calculation with Fee deduction
                size = self.position.size
                profit_usd_raw = size * (price - entry) if self.position.side == 'Long' else size * (entry - price)
                
                fee_rate = getattr(self, 'slippage', 0.0006)
                total_fee_usd = size * entry * fee_rate + size * price * fee_rate
                profit_usd = profit_usd_raw - total_fee_usd
    ...
    def _sync_position_states(self) -> bool:
        """거래소 포지션과 봇 상태 동기화 (v1.6.2 Standard)"""
            ...
            if bot_has_pos and not exchange_has_pos:
                ...
                if valid_pos:
                    ...
                    if not bot_has_pos or not is_bot_order:
                        # [v1.5.9] 로컬에 복구된(load_state) 포지션 정보가 있다면 입양(Adopt) 시도
                        if self.position and self.position.side == side:
                            logging.info(f"[SYNC] ✅ Adopted matching position ({side} @ {entry}). Bridging to internal state.")
                            is_bot_order = True # 관리 대상으로 강제 수용
                        else:
    ...
    def load_state(self):
        ...
        try:
            ...
            # 레거시 방식 (인스턴스 전용 파일 우선)
            target_file = self.state_file if hasattr(self, 'state_file') else STATE_FILE
            if os.path.exists(target_file):
                ...
                if state.get('position'):
                    ...
                    # [v1.5.9] 트레일링 상태(extreme_price 등) 복구
                    if 'extreme_price' in state:
                        self.position.extreme_price = state['extreme_price']
                    if 'trailing_active' in state:
                        self.position.trailing_active = state['trailing_active']
                    ...
                    if state.get('bt_state'):
                        self.bt_state.update(state['bt_state'])
                    logging.info(f"State loaded ({os.path.basename(target_file)}): {self.position.side} @ {self.position.entry_price}, Extreme: {getattr(self.position, 'extreme_price', 'N/A')}")
        except Exception as e:
    ...
    def save_state(self):
        ...
        try:
            state = {
                ...
                'current_capital': (self.exchange.capital if self.exchange else 0), # [NEW] v1.6.2
                'exchange': (self.exchange.name if self.exchange else 'Unknown'),
                'symbol': self.symbol,
                'current_price': self.last_ws_price if self.last_ws_price else 0,
                'extreme_price': getattr(self.position, 'extreme_price', None) if self.position else None,
                'trailing_active': getattr(self.position, 'trailing_active', False) if self.position else False,
    ...
    def manage_position(self):
        """
        포지션 관리 (Alpha-X7 Core 적응형 트레일링 - v1.6.2 WS 최적화)
        """
        if not self.position:
            return
        
        try:
            # [HOTFIX] REST 호출 최소화 (웹소켓 가격 우선)
            # 1초마다 get_klines 호출하던 것을 최적화
            current_price = self.last_ws_price if self.last_ws_price else self.exchange.get_current_price()
            if current_price <= 0:
                return

            # 데이터 로드 (RSI 계산을 위함 - 1분 주기로만 갱신하거나 필요시 호출)
            # 여기서는 캐시를 활용하거나 소량의 데이터만 조회
            df_entry = self.indicator_cache.get('df_entry')
            if df_entry is None or (datetime.now() - self.indicator_cache.get('last_update', datetime.min)).total_seconds() > 60:
            ...
            from core.strategy_core import AlphaX7Core
            if not hasattr(self, '_strategy_core'):
                self._strategy_core = AlphaX7Core(use_mtf=getattr(self, 'USE_MTF_FILTER', True))

            # 현재 가격으로 마지막 봉 업데이트 (실시간 RSI 반영)
            df_entry.iloc[-1, df_entry.columns.get_loc('close')] = current_price
            ...
            # 현재 가격 기반 High/Low 추정 (WS 가격 활용)
            current_high = max(current_price, float(df_entry.iloc[-1]['high']))
            current_low = min(current_price, float(df_entry.iloc[-1]['low']))
            
            # Extreme Price 초기화 (기존 복구된 값이 있으면 유지)
            if not hasattr(self.position, 'extreme_price') or self.position.extreme_price is None:
                self.position.extreme_price = entry
                logging.info(f"[TRAIL] Initialized extreme_price to entry: {entry}")
            else:
                # 이미 점유 중이면 로직에 따라 업데이트되도록 둠
                pass
    ...
    def _close_on_sl(self, exit_price: float):
        ...
        # [v1.5.10] Size-based PnL Calculation with FEE deduction
        size = self.position.size
        pnl_usd_raw = size * (exit_price - entry) if side == 'Long' else size * (entry - exit_price)
        
        fee_rate = getattr(self, 'slippage', 0.0006)
        total_fee_usd = size * entry * fee_rate + size * exit_price * fee_rate
        pnl_usd = pnl_usd_raw - total_fee_usd
                ...
                if self.position:
                    ...
                    try:
                        ...
                        if current_price > 0:
                            ...
                            # [v1.5.10] Size-based PnL for UI status
                            size = self.position.size
                            pnl_usd_raw = size * (current_price - entry) if self.position.side == 'Long' else size * (entry - current_price)
                            
                            # Real-time fee estimate (optional, but good for consistency)
                            fee_rate = getattr(self, 'slippage', 0.0006)
                            total_fee_usd = size * entry * fee_rate + size * current_price * fee_rate
                            pnl_usd = pnl_usd_raw - total_fee_usd
