...
# SSOT: utils.indicators, utils.metrics 사용
from utils.indicators import calculate_rsi, calculate_atr
from utils.metrics import calculate_mdd, calculate_profit_factor
...
# 2. 지표 계산 (SSOT: utils.indicators 사용)
print("\n[2/3] 지표 계산...")
...
print(f"[OK] RSI, ATR 계산 완료 (utils.indicators SSOT)")
...
if len(trades) == 0:
    ...
    # MDD (SSOT: utils.metrics 사용)
    # trades를 표준 형식으로 변환 (pnl_pct → pnl)
    trades_for_metrics = [{'pnl': t['pnl_pct']} for t in trades]
    max_dd = calculate_mdd(trades_for_metrics)

    # 최종 자본
    final_capital = equity_curve[-1]

    # Profit Factor (SSOT: utils.metrics 사용)
    profit_factor = calculate_profit_factor(trades_for_metrics)
