...
# 백테스트 (지표 계산 생략 - run_backtest가 자동 처리)
...
print("\n백테스트 실행 (slippage=0.0002, 지표 자동 계산)...")
...
if len(trades) >= 10:
    ...
    # v7.27 기댓값과 비교
    expected_trades = 3880
    diff = abs(len(trades) - expected_trades)
    print(f"\n기댓값: {expected_trades:,}회")
    print(f"차이: {diff:,}회 ({diff/expected_trades*100:.1f}%)")

    if diff <= 100:
        print("\n[OK] 검증 성공! v7.27 결과 재현 가능")
    else:
        print("\n[FAIL] 검증 실패! 거래 횟수 차이 큼")
