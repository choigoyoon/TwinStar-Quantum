...
from PyQt6.QtWidgets import (
    ...
    QTableView, QHeaderView, QGroupBox,
    QMessageBox
)
...
class MultiBacktestWidget(QWidget):
    ...
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        self.result_model: Optional[MultiSymbolResultModel] = None
    ...
    def _create_result_table(self) -> QTableView:
        """결과 테이블 생성 (QTableView + Model - 10배 성능 향상)"""
        self.result_table = QTableView()
        self.result_table.setStyleSheet(BacktestStyles.table())

        # 빈 모델로 초기화
        self.result_model = MultiSymbolResultModel([])
    ...
    def _run_backtest_thread(self):
        ...
        try:
            ...
            # 백테스트 실행
            self.backtest.set_status_callback(progress_callback)
            results_dict = self.backtest.run()
            
            # results_dict에서 trades 리스트 추출 (UI 호환을 위해)
            results = results_dict.get('trades', [])
    ...
    def _populate_results(self, results: List[Dict[str, Any]]):
        ...
        self.result_model.update_data(results)
